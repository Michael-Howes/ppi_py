...
def ppi_mean_se3(Y, Yhat, Yhat_unlabeled, group, group_unlabeled, lam=1.0):
    n = len(Y)
    ...
    combined_data = np.vstack(
        [
            ...
            np.concatenate(
                [np.zeros(n), (Yhat_unlabeled - Yhat_unlabeled.mean()) / N]
            ),
        ]
    ).T
...
for i in range(reps):
    ...
    theta_hat2, theta_se2 = ppi_mean_se3(
        Y, Yhat, Yhat_unlabeled, group, group_unlabeled
    )
